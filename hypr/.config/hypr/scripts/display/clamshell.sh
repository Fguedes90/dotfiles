...
disable_edp_working_method() {
    ...
    # Verificar se foi detectado um monitor válido
    if [ "$EXTERNAL_MONITOR" = "none" ]; then
    ...
    cat > "$HOME/.config/hypr/conf.d/clamshell-loader.conf" << EOF
...
EOF
    ...
    # Notificar waybar para atualizar seus módulos
    log_message "🔄 Notificando waybar para atualizar para o novo modo..."
    pkill -SIGUSR1 waybar 2>/dev/null || true
    
    # Aguardar um instante e forçar atualização do cursor
    sleep 1
...
EOF
    ...
    if command -v hyprctl &> /dev/null; then
        ...
        # Confirmar monitor externo (se detectado)
        if [ "$EXTERNAL_MONITOR" != "none" ]; then
        ...
        # Workspace 5 sempre no monitor externo (se detectado)
        if [ "$EXTERNAL_MONITOR" != "none" ]; then
